...
/* Mask charging complete interrupt event */
#define NO_CHRCMPL_EVENT
	...
	if (point >= 0 && point < di->chargeConfig->pointsNumber) {
		...
		case DEDICATEDCHARGER:
			...
			CHECK_ERROR(pmic_write_reg(MC34708_REG_USB_CTL,
						   BITFVAL(MUSBCHRG, 3),
						   BITFMASK(MUSBCHRG)));
			val |= BITFVAL(CHRCC, CHRCC_UA_TO_BITS(850000));
			break;
		default:
			val |=
			    BITFVAL(CHRCC,
...
static void usb_charger_online_event_callback(void *para)
{
	...
	if (!ret) {
		usbOnline = BITFEXT(value, USBDETS);
		if (usbOnline) {
			pr_debug("Charger Connected\n");
			kobject_uevent(&di->dev->kobj, KOBJ_ADD);
		}
		else {
			pr_debug("Charger Disconnected\n");
			kobject_uevent(&di->dev->kobj, KOBJ_REMOVE);
		}
	}
}
...
{
	struct ripley_dev_info *di = (struct ripley_dev_info *)para;
	printk("BATT OVER TEMP!!\n");
	ripley_charger_update_status(di);
}

#ifndef NO_CHRCMPL_EVENT
